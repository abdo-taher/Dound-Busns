        ...
        <div class="p-3 container-p-y">
            <div class="card">
                <div class="d-flex align-item-center p-4 justify-content-between w-100">
                    <h5 class="card-header p-0">{{ __('home.branches') }}</h5>
                    <div class="d-flex align-item-center gap-3">
                        <button id="deleteSelected" class="btn btn-danger">
                            <i class="fa-regular fa-trash-can"></i>
                        </button>
                        <a href="{{ route('club.branches.create') }}">
                        ...
                        <tbody>
                            @foreach ($data as $branches)
                                <tr>
                                    ...
                                    <td>{{ $branches->city->name }}</td>
                                    <td>{{ $branches->location }}</td>
                                   <td> {{ number_format($branches->balance, 0) . ' ' . ($branches->club->currency->currency->symbol ) }}</td>
                                    <td class="">
                                        <div class="dropdown">
        ...
        $(document).ready(function() {
            ...
            $('#deleteSelected').click(function() {
                ...
                if (selectedIds.length > 0) {
                    $.ajax({
                        url: "{{ route('club.branches.deleteSelected') }}",
                        type: "POST",
                        data: {
                            ids: selectedIds
                        },
